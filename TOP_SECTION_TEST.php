...
function GET_OEE_OVERRIDE($iConRLM, $retrieve_data, $setup_type = null){
	...
	foreach ($retrieve_data as $key => $rd) {
		...
		$rd_partnum 		= ($setup_type == null) ? "PART" 		 : "MFG_PART_NUM";
		$rd_site_num 		= ($setup_type == null) ? "SITE" 		 : "SITE_NUM";
		$rd_res_area 		= ($setup_type == null) ? "TEST_TYPE" 	 : "RES_AREA";
		$rd_atom_tester 	= ($setup_type == null) ? "ATOM_TESTER"  : "TESTER";
		$rd_atom_handler 	= ($setup_type == null) ? "ATOM_HANDLER" : "HANDLER";

		if (!in_array($rd[$rd_partnum], $partnum)) {
			array_push($partnum, $rd[$rd_partnum]);
		}
		if (!in_array($rd[$rd_site_num], $details)) {
			array_push($details, $rd[$rd_site_num]);
		}
		if (!in_array($rd[$rd_res_area], $details)) {
			array_push($details, $rd[$rd_res_area]);
		}
		...
		if (!in_array($rd[$rd_atom_tester], $details)) {
			array_push($details, $rd[$rd_atom_tester]);
		}
		if (!in_array($rd[$rd_atom_handler], $details)) {
			array_push($details, $rd[$rd_atom_handler]);
		}
	...
	$latest_override = 'SELECT OOMH.*
					...
					JOIN (
						SELECT OOMH2.OEEO_ID, MAX(OOMH2.CHANGE_DT) AS LATEST_DATETIME
						FROM  BRAIN.OEE_OVERRIDE_MAIN_HIST OOMH2
						WHERE OOMH2.OEEO_ID  IN ("'.implode('","', $oeeo_id).'")
						AND (CHANGE_TYPE = "NEW_OVERRIDE" OR CHANGE_TYPE = "UPDATE_OVERRIDE")
						GROUP BY OOMH2.OEEO_ID
					) X
					ON OOMH.OEEO_ID = X.OEEO_ID
					AND OOMH.CHANGE_DT  = X.LATEST_DATETIME;';
			...
			foreach ($identifier as $idtf) {
				...
				foreach ($split as $skey => $sp) {
					if ($skey != 0) {
						...
						switch ($sp) {
							case 'SITE_NUM':
								if ($rd[$rd_site_num] == $oee_overrides[$oee_index][$sp]) {
									$match_counter++;
								}
							break;
							case 'RES_AREA':
								if ($rd[$rd_res_area] == $oee_overrides[$oee_index][$sp]) {
									$match_counter++;
								}
								break;
							case 'TESTER':
								if ($rd['ENGR_TESTER'] == $oee_overrides[$oee_index][$sp] || $rd[$rd_atom_tester] == $oee_overrides[$oee_index][$sp]) {
									$match_counter++;
								}
								break;

							case 'HANDLER':
								if ($rd['ENGR_HANDLER'] == $oee_overrides[$oee_index][$sp] || $rd[$rd_atom_handler] == $oee_overrides[$oee_index][$sp]) {
									$match_counter++;
								}
								break;
