...
// if ($checkRequest === true) {
    ...
    $retrieve_hardware 		= GET_HARDWARE_RECORD_V2($iConRLM, $retrieve_data);
...
// LEFT JOIN BODS_JDA_STAGE.ATOM_MASTER AM ON AAA.AM_HASH = AM.AM_HASH
    ...
    $strSQL = "
        ...
        WHERE AAA.MFG_PART_NUM IN ('".implode("','",$aryTemp)."') AND AAA.GENERIC IS NOT NULL";
...
function GET_UNPLANNABLE($iConRLM, $data){
	...
	if (count($data) > 0) {
		...
		foreach ($data as $key => $dt) {
			array_push($aaa_id, $dt['AAA_ID']);
		}
		$query = 'SELECT * FROM TEST.ADI_UNPLANNABLE_SETUP WHERE AAA_ID IN ("'.implode('","', $aaa_id).'")';
		$result = ExecuteIQuery($query,$iConRLM);
...
function GET_DEDICATION($iConRLM, $data){
	...
	foreach ($result as $key => $res) {
		...
		$get_prio = 'SELECT ORIG_PRIO, NEW_PRIO FROM TEST.ADI_SETUP_PRIO WHERE IDENTIFIER = "'.$res['IDENTIFIER'].'"';
		$get_prio_res = ExecuteIQuery($get_prio,$iConRLM);
		while($row = mysqli_fetch_assoc($get_prio_res)){
			$result[$key]['ORIG_PRIO'] = $row['ORIG_PRIO'];
			$result[$key]['PRIO_CD'] = $row['NEW_PRIO'];
		}
	}
	return $result;
}
